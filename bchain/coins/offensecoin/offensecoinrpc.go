...
import (
	...
	"github.com/OFFENSE-COIN/blockbook/bchain"
	"github.com/OFFENSE-COIN/blockbook/bchain/coins/btc"
	"github.com/OFFENSE-COIN/blockbook/common"
)

// offensecoinRPC is an interface to JSON-RPC bitcoind service
type offensecoinRPC struct {
	*btc.BitcoinRPC
}
...
// NewoffensecoinRPC returns new offensecoinRPC instance
func NewoffensecoinRPC(config json.RawMessage, pushHandler func(bchain.NotificationType)) (bchain.BlockChain, error) {
...
// Initialize initializes offensecoinRPC instance
func (z *offensecoinRPC) Initialize() error {
	...
	z.Parser = NewoffensecoinParser(params, z.ChainConfig)
...
func (z *offensecoinRPC) GetChainInfo() (*bchain.ChainInfo, error) {
...
func (z *offensecoinRPC) GetBlock(hash string, height uint32) (*bchain.Block, error) {
	var err error
	if hash == "" && height > 0 {
...
func (z *offensecoinRPC) GetTransactionForMempool(txid string) (*bchain.Tx, error) {
	return z.GetTransaction(txid)
}

// GetMempoolEntry returns mempool data for given transaction
func (z *offensecoinRPC) GetMempoolEntry(txid string) (*bchain.MempoolEntry, error) {
